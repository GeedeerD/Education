...
namespace DataBase
{
    public class MessageRepository
    {
        ...
        public IEnumerable<MessageModel> GetAllMessages (IndividualMessageFilter filter)
        {
            ...
            result.AddRange(_db.GetCollection<MessageModel>("Messages").AsQueryable().Where(x => x.FromName == filter.FromName || x.ToName == filter.ToName).ToList().OrderBy(x => x.SentAt));
